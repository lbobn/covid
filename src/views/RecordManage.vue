<template>
  <div>
    ...
    <el-table :data="tableData" border style="width: 100%">
      <el-table-column fixed prop="id" label="编号" sortable width="120">
      </el-table-column>
      <el-table-column prop="name" label="姓名" width="120"> </el-table-column>
      <el-table-column prop="sex" label="性别" width="50"> </el-table-column>
      <el-table-column prop="phonenum" label="手机号码" width="120">
      </el-table-column>
      <el-table-column label="体温" prop="temp" width="80" sortable>
        <template slot-scope="scope">
          <div slot="reference">
            <p
              size="medium"
              :class="scope.row.temp > 36 && scope.row.temp < 37.2 ? '' : 'red'"
            >
              {{ scope.row.temp + "°C" }}
            </p>
          </div>
      ...
      <el-table-column prop="risk" label="是否到达过高风险地区" width="120">
      </el-table-column>
      <el-table-column prop="health" label="健康状况" width="170">
      </el-table-column>
      <el-table-column prop="createTime" label="打卡时间" width="170" sortable>
      </el-table-column>
...
export default {
  ...
  methods: {
    reload(){
      axios
        .get(
          "http://localhost:8080/emp/findAll/" +
            this.currentpage +
            "/" +
            this.pagesize
        )
    ...
    search() {
      if (!this.stext) {
        ...
        axios
          .get(
            "http://localhost:8080/emp/search/" +
              this.searchKey +
              "/" +
              this.stext
          )
    ...
    deleteRecord(row) {
      this.$confirm("是否确定要删除" + row.name + "的打卡记录?", "删除数据", {
        ...
        axios
          .delete("http://localhost:8080/emp/deleteById/" + row.id)
          .then((resp) => {
  ...
  created() {
    axios.get("http://localhost:8080/emp/findAll/1/5").then((resp) => {
      console.log(resp);
      this.tableData = resp.data.data.records;
      this.total = resp.data.data.total;
    });
